...
public class OwnersFacadeImplTest {
    ...
    public void should_find_owners() {
        // given
        OwnersFacade ownersFacade = new OwnersFacadeImpl(aDataSource().withDataScript("owner/insert-owners.sql").build());

        // when
        List<OwnerDto> owners = ownersFacade.findOwners();

        // then
        assertThat(owners).containsExactly(new OwnerDto("Robert"), new OwnerDto("Laure"));
    }
